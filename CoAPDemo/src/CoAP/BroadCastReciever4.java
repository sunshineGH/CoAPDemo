 ...
 */  
//节点睡眠周期结束后会发送一个awake报文给代理，采用的是双信道的方式，即节点在睡眠时仍然可以有一个低功率信道监听代理发来的awake报文
public class BroadCastReciever4  
{  
	     int sleepState;
	     ...
	     public BroadCastReciever4() throws Exception {
	     ...
	     public BroadCastReciever4(InetAddress address,int sleepState,int sleepDuration,int timeSleeping,int nextSleep,boolean isRecieve) throws Exception {
	     ...
	     public BroadCastReciever4(int sleepState,int sleepDuration,int timeSleeping,int nextSleep) throws Exception {
	    	 unicastSocket = new DatagramSocket();
	    	 ms = new MulticastSocket(5683);
	    	 isRecieve = false;
	    	 this.sleepState = sleepState;//val=1,表示sleep;val=0,表示awake
	    ...
	    /*public static class Sleep {
	    	 Timer timer;
	    	 public Sleep(BroadCastReciever4 reciever) {
	    		 timer = new Timer();
	     ...
	     public static class Awake {
	    	 Timer timer;
	    	 public Awake(BroadCastReciever4 reciever) {
	    		 timer = new Timer();
	    		 timer.schedule(new TimerTask() {
	    			 public void run() {
							try {
      ...
      {  
    	  BroadCastReciever4 reciever = new BroadCastReciever4();
    	 /* DatagramSocket unicastSocket = new DatagramSocket();
    	  MulticastSocket ms = new MulticastSocket(5683);
